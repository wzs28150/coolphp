...
class Debris extends Common
{
    ...
    public function add(){
        if(request()->isPost()) {
            ...
            $info1=array('setup'=>array ( 'edittype' => 'UEditor', ),'pattern'=>'defaul','name'=>'内容','field'=>'content');
            $form=new Form($info1);
            $this->assign ( 'form', $form );
            $returnData['form'] = $form;
             $r=array("id"=>6,"moduleid"=>1,"field"=> "content","name"=>"内容","pattern"=>"defaul","class"=>"content","type"=>"editor","setup"=>array('edittype' => 'UEditor'));
            $this->assign ( 'r', $r );
    ...
    public function edit(){
        if(request()->isPost()) {
           ...
           // echo db('debris')->getlastsql();exit;
            $result['code'] = 1;
            $result['msg'] = '碎片修改成功!';
            $result['url'] = url('index');
            return $result;
