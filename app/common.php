...
function savecache($name = '', $id='')
{
    if ($name=='Field') {
        ...
        $grouplist = db('auth_group')->field('group_id,rules')->select();

        foreach ($grouplist as $key => $value) {
          $group_id = $value['group_id'];
          $authstr = rtrim($value["rules"],',');

          $authlist = db('auth_rule')->field('catid')->where('id','in',$authstr)->where('catid','<>','')->fetchSql(false)->select();
          //dump($authlist);exit;
          $catidstr ='';
          foreach ($authlist as $key => $value) {
            $catidstr.=$value['catid'].',';
          }
          $catidstr = rtrim($catidstr,',');

          $Model = db($name);
          if($catidstr){
            $list = $Model->where('id','in',$catidstr)->order('listorder')->fetchSql(false)->select();
          }else{
            $list = $Model->order('listorder')->fetchSql(false)->select();
          }
          $pkid = $Model->getPk();
          $data = array();
          foreach ($list as $key => $val) {
