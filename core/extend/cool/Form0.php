...
class Form{
    ...
    public function title($info,$value){
        ...
        if($info['required']==1){
            $parseStr   = '<input type="text" name="'.$field.'" data-required="'.$info['required'].'" value="'.$value.'" data-min="'.$info['minlength'].'" data-max="'.$info['maxlength'].'" data-errormsg="'.$info['errormsg'].'" title="'.$name.'" placeholder="请输入'.$name.'(必填)" lay-verify="defaul'.$pattern.'" class="'.$info['class'].' layui-input"/> ';
        }else{
        ...
        $stylestr .='<div class="layui-input-block"><input type="text" name="style_color" id="style_color" value="'.$style_color.'"/></div></div>';
        $stylestr .='<div class="layui-form-item"><label class="layui-form-label">加粗</label>';
        $stylestr .='<div class="layui-input-block"><input type="checkbox" name="style_bold" value="bold" '.$boldchecked.' title="加粗">';
        //缩略图
        $thumbstr ='</div></div>';
        $thumbstr .='<div class="layui-form-item"><label class="layui-form-label">缩略图</label>';
        $thumbstr .='<div class="layui-input-block"><input type="hidden" name="thumb" id="thumb" value="'.$title_thumb.'"><div class="layui-upload">';
    ...
    public function datetime($info,$value){
        ...
        if($info['required']==1){
            $parseStr = '<input type="datetime" title="'.$name.'" name="'.$field.'" data-required="'.$info['required'].'" placeholder="请输入'.$name.'(必填)" value="'.$value.'" class="'.$info['class'].' layui-input" id="addtime">';
        }else{
